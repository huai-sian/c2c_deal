...
export default function Navbar() {
  ...
  const { products, allSeries, wish, wishLength } = useSelector(
    (store) => store.products
  );
  const { cart, cartApi, cartlength, total } = useSelector(
    (store) => store.cart
  );
        ...
        <nav className="navbar">
          <Link to="/" className="nav-link">
            <span className="nav-title">
              <i className="fas fa-praying-hands"></i>Pure Savon
            </span>
          </Link>
        </nav>
                  ...
                  {wish &&
                    wish.map((item, i) => (
                      <tr className="pb-0" key={item.id}>
                        ...
                        <td
                          width="50%"
                          className="wish_title"
                          onClick={() =>
                            history.push(`/product_detail/${item.id}`)
                          }
                        >
                          {item.title}
                        </td>
            ...
            <div className="dropdown-menu dropdown-menu-right dropdown-menu-width p-2">
              ...
              <div className="totalinfo">
                ...
                <div className="btn btn-goCart">
                  <div onClick={() => history.push(`/productlist`)}>
                    {t("gotoshop")}
                  </div>
                </div>
                <div className="btn btn-goCart">
                  <div onClick={() => history.push(`/checkorders`)}>
                    {t("gocheck")}
                  </div>
                </div>
              </div>
